...
object ImageDrawingService: Loggable {
            ...
            models.forEach {
                launch {
                    try {
                        ...
                        logger.info("Sending request to model=${it.key} with prompt=$description")
                        val response = client.newCall(request).execute()
                        val status = response.code()
                        val body = response.body()
                        if (status != SC_OK) {
                            logger.warn("ImageDraw response from model=${it.key} came with status=$status and body=${body.string()}")
                            return@launch
                        }
                        val bytes = body.bytes()

                        logger.info("Successful drawing operation for model=${it.key}")
                        responses[it.key] = Image.raw(bytes, Image.Format.JPEG)
                    } catch (e: Exception) {
                        logger.error("An error occured on model=${it.key}, ignoring it", e)
                    }
                }
            }
        }
